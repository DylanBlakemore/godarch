...
- [ ] Add the GitHub Actions CI matrix; get it green on all three OSes. _(workflow committed at `.github/workflows/ci.yml` — 3-OS matrix, cgo, fmt-check, lint, build, vet, race; the equivalent gates pass locally on macOS. Green-on-all-three confirmed only on first push to GitHub.)_
